...
print(Cld_sim)
Cld_sim= as.data.frame.list(Cld_sim$`sample_data(rare_phylo)$Vegetation`)
Cld_sim <- tibble::rownames_to_column(Cld_sim, "Vegetation")

Cld_sh=multcompLetters4(anova.sh.veg, tukey.sh)
print(Cld_sh)
Cld_sh= as.data.frame.list(Cld_sh$`sample_data(rare_phylo)$Vegetation`)
Cld_sh <- tibble::rownames_to_column(Cld_sh, "Vegetation")
...
#shannon plot
alpha_mo_sh <- plot_richness(rare_phylo, x = "Vegetation", measures = c("Shannon"))
fun_alpha_sh <- alpha_mo_sh + 
  ...
  geom_text(data = Cld_sh,
            aes(x=Vegetation,y=5,label=monospacedLetters),
            vjust = 1, 
            hjust = .2)+
 scale_fill_manual(values=c("Mesquite" ="#c1292e", "Ecotone" = "#e1bc29", "Grass" = "#679436", name = "Vegetation"))

fun_alpha_sh


#simpson plot
alpha_mo_sim <- plot_richness(rare_phylo, x = "Vegetation", measures = c("Simpson"))
alpha_mo_sim

fun_alpha_sim <- alpha_mo_sim + 
  geom_boxplot(show.legend = FALSE, aes(fill=Vegetation)) +
  theme_linedraw()+
  stat_summary(fun=mean, geom="point", shape=18, size = 2, color="lightgray") +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))+
  ...
  geom_text(data = Cld_sim,
            aes(x=Vegetation,y=1,label=monospacedLetters),
            vjust = 1, 
            hjust = .2)+
 scale_fill_manual(values=c("Mesquite" ="#c1292e", "Ecotone" = "#e1bc29", "Grass" = "#679436", name = "Vegetation"))
...
#merge figures into one
 library(ggpubr)
figure <- ggarrange(fun_alpha_sh, fun_alpha_sim, 
          ncol = 2)
figure

final <- annotate_figure(figure,
              top = text_grob("Fungal Alpha Diversity", color = "black", face = "bold", size = 12),
              left = text_grob("Alpha Diversity Measure", color = "black", rot = 90), 
              bottom = text_grob("Month", color = "black"))
              
final

#ggsave(file="~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/images/fun_alpha_with_letters.png", device = png, plot=final, width=7, height=5)
