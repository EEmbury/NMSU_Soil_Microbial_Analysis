...
#write.csv(taxa,"~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/fungal_indicators_multipatt_taxa_all.csv")
...
### FUNGuild assessment of indicator species
...
# taxa_final <- taxa[, -c(2:13)] 
# #3. Write a csv 
# 
write.csv(taxa, "~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Fungi/FunGuild_indicspecies.csv", row.names = FALSE)
# 
# #Proceed to running FUNGuild on command line using the 'FunGuild.csv' file as .txt


# python3 Guilds_v1.1.py -otu FunGuild_indicspecies.txt -db fungi -m -u
```

Found 100 matching taxonomy records in the database.
Dereplicating and sorting the result...
FunGuild tried to assign function to 76 OTUs in 'FunGuild_indicspecies.txt'.
FUNGuild made assignments on 59 OTUs.
Result saved to 'FunGuild_indicspecies.guilds.txt'

Additional output:
FUNGuild made assignments on 59 OTUs, these have been saved to FunGuild_indicspecies.guilds_matched.txt.
17 OTUs were unassigned, these are saved to FunGuild_indicspecies.guilds_unmatched.txt.



NEED TO ADD THE DEREPLICATED TAXA BACK TO SPREADSHEET

```{r}
#read in guild data
guilds <- read.csv("~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Fungi/FunGuild_indicspecies.guilds2.csv", header = TRUE)

guild_indic_data <- merge(guilds, taxa, 
                          by = 'sample.id', all = FALSE)


#write.csv(guild_indic_data, "~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Fungi/indicspecies_with_guild.csv", row.names = FALSE)

# count occurrence of each trophic mode by vegetation type
guild_taxa <- read.csv("~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Fungi/indicspecies_with_guild.csv", header = TRUE)

agg_df <- aggregate(guild_taxa$Trophic.Mode, by=list(guild_taxa$Vegetation, guild_taxa$Trophic.Mode), FUN=length)
agg_df
#write.csv(agg_df, "~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Fungi/indicspecies_count_per_trophic_mode.csv", row.names = FALSE)
