...
# normalize data with proportions (aka Total Sum Normalization (TSS))
# package = microbiomeMarker
phylo_tss <- normalize(phylo_rm_contaminat_rm_cntrl, method = "TSS")

# calculate Bray-Curtis distance using the vegan package
DistBC <- distance(phylo_tss, method = "bray")

#ordinate bray distances
ordBC <- ordinate(phylo_tss, method = "NMDS", distance = DistBC)

#plot distances
plot_ordination(phylo_tss, ordBC, color = "vegetation") + 
  geom_point() +
...
sampleDF <-  data.frame(sample_data(phylo_tss))

# adonis : Analysis of variance using distance matrices
adonis2(DistBC ~ sampleDF$vegetation, data = sampleDF)

library(pairwiseAdonis)
pairwise.adonis(DistBC, phyloseq::sample_data(phylo_tss)$vegetation)
