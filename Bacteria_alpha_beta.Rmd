...
#normality assumptions don't seem great
anova.sh.veg.sq = aov(((richness$Shannon)^2) ~ sample_data(rare_phylo)$Vegetation*sample_data(rare_phylo)$Month)
summary(anova.sh.veg.sq)

#post hoc
tukey.sh <- TukeyHSD(anova.sh.veg.sq)
tukey.sh$`sample_data(rare_phylo)$Month`

#check anova assumptions after log transform
par(mfrow = c(1,2)) #puts the plots next to each other
qqnorm(anova.sh.veg.sq$residuals) #normal q-q plot, tests normality
qqline(anova.sh.veg.sq$residuals, col = "red") #line of fit on q-q plot
plot(anova.sh.veg.sq$fitted.values, anova.sh.veg.sq$residuals, main = "Residuals vs Fitted", xlab = "Fitted Values", ylab = "Residuals") #residuals vs. fitted, test variance
abline(h=0, col = "red") 
...
#library(svglite)
#ggsave(file="beta.svg", plot=p, width=7, height=7)
...
adonis2(DistBC ~ sampleDF$Vegetation*sampleDF$Month, data = sampleDF, strata = sampleDF$proximity, nperm = 999)
...
library(pairwiseAdonis)
pairwise_adon <- pairwise.adonis(DistBC, phyloseq::sample_data(rare_phylo)$month_veg)


month_veg = sample_data(rare_phylo)$month_veg
proximity1 = sample_data(rare_phylo)$proximity1

pairwise.adonis2(DistBC ~ month_veg, strata = "proximity1", data= data.frame(sample_data(rare_phylo)))
