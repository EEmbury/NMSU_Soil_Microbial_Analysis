...
#load otu tables and taxa table
...
#getiing tukey letters for plot
library(multcompView)
Cld_sim=multcompLetters4(anova.sim.veg, tukey.sim)
print(Cld_sim)
Cld_sim= as.data.frame.list(Cld_sim$`sample_data(rare_phylo)$Month`)
Cld_sim <- tibble::rownames_to_column(Cld_sim, "Month")

Cld_sh=multcompLetters4(anova.sh.veg, tukey.sh)
print(Cld_sh)
Cld_sh= as.data.frame.list(Cld_sh$`sample_data(rare_phylo)$Month`)
Cld_sh <- tibble::rownames_to_column(Cld_sh, "Month")
...
x <- alpha_veg_sh + geom_boxplot() 
x + geom_text(data = Cld_sh,
            aes(x=Month,y=10,label=Letters))


#plot shannon 
alpha_mo_sh <- plot_richness(rare_phylo, x = "Month", measures = c("Shannon"))
bac_alpha_sh <- alpha_mo_sh + 
  geom_boxplot(show.legend = FALSE, aes(fill=Month)) +
  theme_linedraw()+
  stat_summary(fun=mean, geom="point", shape=18, size = 2, color="lightgray") +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))+
theme(axis.title.x = element_blank(), axis.text.x = element_text(), axis.ticks.x   = element_blank(), axis.title.y = element_blank()) + 
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.001))+ 
  geom_text(data = Cld_sh,
            aes(x=Month,y=8,label=Letters),
            vjust = 2,
            hjust =0.2)+
...
#plot simpson 
alpha_mo_sim <- plot_richness(rare_phylo, x = "Month", measures = c("Simpson"))
alpha_mo_sim
bac_alpha_sim <- alpha_mo_sim + 
  geom_boxplot(show.legend = FALSE, aes(fill=Month)) +
  theme_linedraw()+
  stat_summary(fun=mean, geom="point", shape=18, size = 2, color="lightgray") +
  theme(plot.title = element_text(face = "bold", hjust = 0.5))+
  ...
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.001))+ 
  geom_text(data = Cld_sim,
            aes(x=Month,y=1,label=monospacedLetters),
            vjust = 2, 
            hjust = .2)+
  scale_fill_manual(values=c("October" ="#c1292e", "January" = "#e1bc29", "March" = "#679436", "May" = "#4895ef" , "July" ="#8187dc"), name = "Month") #add custom colors and change legend name
bac_alpha_sim 
...
figure <- ggarrange(bac_alpha_sh, bac_alpha_sim, 
          ncol = 2)
figure

final <- annotate_figure(figure,
              top = text_grob("Bacterial Alpha Diversity", color = "black", face = "bold", size = 12),
              left = text_grob("Alpha Diversity Measure", color = "black", rot = 90), 
              bottom = text_grob("Month", color = "black"))
              
final

#ggsave(file="~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/images/bac_alpha_with_letters.png", device = png, plot=final, width=7, height=5)
```
