...
#Simpson
anova.smp.veg = aov(richness$Simpson ~ sample_data(rare)$vegetation*sample_data(rare)$month)
summary(anova.smp.veg)

tukey.smp <- TukeyHSD(anova.smp.veg)

alpha_veg_simp <- plot_richness(rare, x = "vegetation", measures = c("Simpson"))
alpha_veg_simp

alpha_mo_simp <- plot_richness(rare, x = "month", measures = c("Simpson"))
alpha_mo_simp

#Chao1
anova.ch.veg = aov(richness$Chao1 ~ sample_data(rare)$vegetation*sample_data(rare)$month)
summary(anova.ch.veg)

tukey.ch <- TukeyHSD(anova.ch.veg)

alpha_veg_ch <- plot_richness(rare, x = "vegetation", measures = c("Chao1"))
alpha_veg_ch

alpha_mo_ch <- plot_richness(rare, x = "month", measures = c("Chao1"))
alpha_mo_ch
