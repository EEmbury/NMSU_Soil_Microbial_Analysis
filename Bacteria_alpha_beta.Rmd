...
biom_data_rare <- import_biom(BIOMfilename = "Bacteria/bacteria_taxa_table_rare.biom", refseqfilename = "Bacteria/bacteria-dna-sequences-rare.fasta")
...
phylo_rare <- merge_phyloseq(biom_data_rare, mapping_file)
#Add names to biom table and check phyloseq objects
colnames(tax_table(phylo_rare))= c("Kingdom","Phylum","Class","Order","Family","Genus", "Species")
rank_names(phylo_rare)

depths <- sample_sums(phylo_rare)
```
...
DistBC <- distance(phylo_rare, method = "bray")

#ordinate bray distances
ordBC <- ordinate(phylo_rare, method = "NMDS", distance = DistBC)

#plot distances
plot_ordination(phylo_rare, ordBC, color = "vegetation") + 
  geom_point() +
...
sampleDF <-  data.frame(sample_data(phylo_rare))

# adonis : Analysis of variance using distance matrices
adonis2(DistBC ~ sampleDF$vegetation, data = sampleDF)

library(pairwiseAdonis)
pairwise.adonis(DistBC, phyloseq::sample_data(phylo_rare)$vegetation)
