...
**</span>'
subtitle: '<span style = ''font-size:12pt;''> Bacteria CCA
</span>'
author: "Emily Embury"
date: "`r Sys.Date()`"
...
```{r}
ps_rel <- transform_sample_counts(phylo_rm_archaea, function(x) x / sum(x))

otu_all <- as.data.frame(otu_table(ps_rel)) 

row.names.remove <- c("CONTROL1", "CONTROL2", "CONTROL3" , "129", "93")

sam_data <- sam[!(row.names(sam) %in% row.names.remove), ]

samdata <- sam_data[-c(2:10, 13, 14, 16, 17)]

df <- rownames_to_column(otu_all, var = "sample.id")

merged <- merge(df, samdata, "sample.id")

colnames(samdata) <- c("sample.id", "Soil Temp", "Soil pH", "C:N", "Avg. Air Temp", "Avg. Humid.", "Avg. ppt", "month_veg", "Vegetation", "Month")
```
...
mod_all_veg <- cca(otu_all ~ Vegetation*(`Soil Temp` + `Soil pH` + `C:N` + `Avg. Air Temp` + `Avg. Humid.` + `Avg. ppt`), samdata)

aov_veg <- anova(mod_all_veg, by="term", permutations=999)
aov_veg

write.table(aov_veg, file = "~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Bacteria/bac_cca_veg_aov.txt", sep = ",", quote = FALSE, row.names = T)


#model to assess month interaction
mod_all_month <- cca(otu_all ~ Month*(`Soil Temp` + `Soil pH` + `C:N` + `Avg. Air Temp` + `Avg. Humid.` + `Avg. ppt`), samdata)

aov_month <- anova(mod_all_month, by="term", permutations=999)
aov_month

write.table(aov_month, file = "~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/Bacteria/bac_cca_month_aov.txt", sep = ",", quote = FALSE, row.names = T)
...
geom_text(data = ccavectors, aes(x = 1.25*CCA1, y = 1.25*CCA2, label = rownames(ccavectors)), size=3) +
  labs(title = "Bacterial Canonical Correspondence Analysis - Vegetation") +
  theme_linedraw()+
...
plot_cca_veg
library(svglite)
ggsave(file="~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/images/bac_cca_veg.png", device = png, plot=plot_cca_veg, width=7, height=5)
...
plot_cca_month <- ggplot(site_data) +
  ...
  scale_y_continuous(limits = c(-5, 5)) +
 geom_text(data = ccavectors, aes(x = 1.25*CCA1, y = 1.25*CCA2, label = rownames(ccavectors)), size=3) +
  labs(title = "Bacterial Canonical Correspondence Analysis - Month") +
  theme_linedraw()+
    theme(plot.title = element_text(face = "bold", hjust = 0.5))+
...
plot_cca_month
library(svglite)
ggsave(file="~/Desktop/Master_Data/Masters_R/NMSU_Soil_Microbial_Analysis/images/bac_cca_month.png", device = png, plot=plot_cca_month, width=7, height=5)
